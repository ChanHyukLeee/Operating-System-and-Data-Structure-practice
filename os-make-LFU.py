...
if options.solve == False:
    ...
    # make dictionary for checking frequency in LFU
    addr_list1 = []
    for i in range(len(addrList)):
        addr_list1.append(int(addrList[i]))
    LFU_dict = dict.fromkeys(addr_list1, 0)
    ...
    for nStr in addrList:
        ...
        try:
            ...
            if policy == 'LFU':
                # update dictionary 
                LFU_dict[n] = LFU_dict[n] + 1
                memory.remove(n)
                for i in range(len(memory)-1, 0, -1):
                    if LFU_dict[n] >= LFU_dict[memory[i]]:
                        memory.insert(i+1, n)
                    else:
                        memory.insert(0,n)
        # when miss    
        ...
        if idx == -1:
            ...
            if count == cachesize:
                ...
                # policy = LFU when miss
                elif policy == 'LFU':
                    LFU_dict[n] = LFU_dict[n] + 1
                    if LFU_dict[memory[0]] < LFU_dict[n]:
                        for i in range(len(memory)-1, 0, -1):
                            if LFU_dict[n] > LFU_dict[i]:
                                memory.insert(i+1,n)
                                victim = memory.pop(0)
                                break
                    elif LFU_dict[memory[0]] == LFU_dict[n]:        
                        if LFU_dict[memory[len(memory)-1]] == LFU_dict[n]:
                            
                            memory.append(n)
                            victim = memory.pop(0)
                        else:
                            for i in range(len(memory)):
                                if LFU_dict[n] >= LFU_dict[memory[i]] and LFU_dict[n]<LFU_dict[memory[i+1]]:     
                                    memory.insert(i+1, n)
                                    victim = memory.pop(0)
                                    break
                    else:
